...
export async function buildStoryPdfBuffer(draft) {
  ...
  let portraitBase64 = isPremium ? draft.characterImageBase64 || null : null;
  if (!isPremium && draft.characterImageUrl) {
    portraitBase64 = await fetchImageAsBase64(draft.characterImageUrl);
  }
  ...
  // ---- Story pages ----
  // Both tiers now produce the same page shape ({ text, image }) — see
  // README "Basic tier is now illustrated too". Premium's images come from
  // a custom photo character; Basic's from the cached library-character
  // pose set — but by the time this function runs, that distinction is
  // already resolved into identical data.
