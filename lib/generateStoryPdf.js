...
// Full-bleed "cover fit" image: scales a source image (always generated
// square, 1024x1024, by every route in this app) up so it fully covers
// the page regardless of orientation, cropping the overflow via a clip
// path — the same clip technique proven for the old circular portrait,
// generalized to the full page rectangle. Falls back to a solid
...
// page.
function drawFullBleedImage(doc, theme, base64, pageWidth, pageHeight, fallbackInitial) {
  if (!base64) {
  ...
  try {
    ...
    doc.clip();
    doc.discardPath();
    const scale = Math.max(pageWidth / 1024, pageHeight / 1024);
    const drawSize = 1024 * scale;
    const offsetX = (pageWidth - drawSize) / 2;
    const offsetY = (pageHeight - drawSize) / 2;
    doc.addImage(`data:image/png;base64,${base64}`, "PNG", offsetX, offsetY, drawSize, drawSize);
...
export async function buildStoryPdfBuffer(draft) {
  ...
  drawFullBleedImage(doc, theme, coverBase64, pageWidth, pageHeight, displayName?.[0]);
  ...
  const backCoverBase64 = draft.backCoverImageUrl
    ...
    : coverBase64;
  drawFullBleedImage(doc, theme, backCoverBase64, pageWidth, pageHeight, displayName?.[0]);
