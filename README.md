...
**Two more corrections, same theme:** the "Meet a few of the characters" grid on the landing page and the full `/characters` browser both used `object-cover` on a square container with a square source image — mathematically zero cropping, but the character still filled the frame edge-to-edge with no breathing room, since the source images themselves have little margin around the figure. Both now have `p-3` (or `p-2` for the smaller character-detail modal) padding on the image container plus `object-contain`, giving visible space around every character regardless of the source image's own composition — this fixes it immediately for already-cached characters too, not just ones generated after the pose-prompt fix above.
