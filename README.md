...
**Scrolling character banner:** the landing page hero now has an infinite horizontal marquee of all 30 characters with a small 👋 badge on each, giving the "happy characters saying hi" welcome effect. Deliberately built to use only already-cached character images (via the existing `/api/library-characters` read, which never triggers generation) rather than fetching a "happy" or "waving" pose specifically — a marketing banner shown to anonymous, often bot/crawler traffic is the wrong place to spend real image-API money warming a cache. Uncached characters just show their initial, same fallback used everywhere else in the app; the banner naturally fills in with real portraits over time as characters get used elsewhere. The animation pauses on hover and respects `prefers-reduced-motion` (inherited from the site's existing global reduced-motion rule).
...
- [ ] Landing page banner: confirm the character marquee scrolls smoothly and loops without a visible jump or gap, and that hovering over it pauses the animation
