...
**Scrolling character banner:** the landing page hero now has large character portraits sliding behind the headline and buttons at low opacity (16%), rather than a separate row of icons below the text. Each portrait is sized at exactly `33.334vw` (one third of the viewport width), so precisely 3 are visible on screen at once regardless of screen size — this was a deliberate second-pass revision after the first version (a row of small circular avatars below the text) didn't read as "background," just as another UI element competing for attention. Deliberately built to use only already-cached character images (via the existing `/api/library-characters` read, which never triggers generation) rather than fetching a "happy" or "waving" pose specifically — a marketing banner shown to anonymous, often bot/crawler traffic is the wrong place to spend real image-API money warming a cache. Uncached characters show a giant faint initial, which at 16% opacity reads as texture rather than an obvious placeholder. The animation respects `prefers-reduced-motion` (inherited from the site's existing global reduced-motion rule); hover-to-pause only works over the parts of the hero not covered by the foreground text, since the text sits in front of (and above, via `z-10`) the background layer.
...
- [ ] Landing page banner: confirm exactly 3 character portraits are visible in the hero background at typical desktop width, that they scroll smoothly and loop without a visible jump, and that the hero text stays clearly readable over them at the current 16% opacity
