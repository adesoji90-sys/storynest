...
**Hero background — third revision, now a slow crossfade of 2:** went through two earlier versions (a row of small avatars below the text, then 3 large portraits continuously sliding behind it) before landing here. Feedback on both: the sliding version still read as a moving UI element rather than calm background texture, and a square source image inside a taller-than-wide slot was cropping the character's head on some screen proportions. Now: exactly 2 character portraits at a time, at 16% opacity, crossfading to a new pair every 7 seconds (`useRotatingCharacters()` — a plain interval advancing a start index through the character list, no library needed). Two deliberate CSS choices fix the head-crop specifically: `object-position: top` instead of the default center, so if a square image ever needs to crop vertically to fill its slot, it always sacrifices the feet, never the head — this holds regardless of the viewer's exact screen proportions, unlike trying to get the container math exactly right. And a slower, dedicated `.animate-fade-in-slow` (1.4s ease-in-out) distinct from the existing `.animate-fade-in` (0.4s) used by the generation-progress modal's rotating facts — that one's tuned for quick rotation, this needed to feel calmer. Still uses only already-cached images via `/api/library-characters` (a pure cache read, never triggers generation) for the same reason as before: anonymous, often bot/crawler landing-page traffic is the wrong place to spend real image-API money warming a cache.
...
- [ ] Character framing: generate a character that hasn't been cached yet and confirm the whole head and body are visible, not cropped at the top — this was a real reported bug (prompt fix applied, not yet visually re-verified)
- [ ] Landing page banner: confirm exactly 2 character portraits crossfade in the hero background every ~7 seconds, that the transition feels gentle rather than abrupt, and that no character's head appears cropped regardless of browser window width (resize to test a few widths — this is exactly the class of bug `object-top` was added to prevent)
